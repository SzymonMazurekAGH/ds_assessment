...
def compute_tsne(
    data: np.array, perplexity: int = None, n_iterations: int = 5000, seed: int = 0
) -> np.array:
...
def compute_clustering_metrics(embedding: np.array, labels) -> dict[str, float]:
    ...
    metric_dict["Calinski-Harabasz Index"] = str(
        calinski_harabasz_score(embedding, labels) / len(labels)
    )
...
def plot_embedding(
    embedding: np.array, labels: np.array, class_to_idx: str, save_path: str = None
):
    ...
    scatter = ax.scatter(
        embedding[:, 0], embedding[:, 1], c=labels, cmap="Spectral", s=10
    )
    plt.legend(
        handles=scatter.legend_elements()[0], labels=reverse_dict(class_to_idx).values()
    )
...
def sample_from_dataset(dataset, num_samples: int = 1000, replacement: bool = False):
    ...
    """
    random_samples = torch.utils.data.RandomSampler(
        dataset, num_samples=num_samples, replacement=replacement
    )
